...
| **Analyzer** | 对采集的内容（raw_description、stars、topics 等）进行摘要、评分和分类，生成结构化 JSON；只做分析，不写入文件系统 | 原始数据文件（JSON，从 raw/ 读取） | 返回结构化 JSON（分析结果由 Organizer 写入） |
| **Organizer** | 读取 Analyzer 输出，进行质量校验（JSON Schema 校验必须字段完整性）、数据格式化、文件写入（articles/）、分发队列管理 | Analyzer 输出的结构化 JSON | articles/ 目录下分发就绪的知识条目 |

### 6.1 Pipeline 执行顺序

```
Collect → Save（raw/） → Load → Deduplicate（按 source_url） → Analyze（LLM） → Organize → Save articles
```

去重在 Analyze 之前执行（从 raw/ 加载后），确保重复条目不会浪费 LLM token。
